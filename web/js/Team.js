...
// Model for Team entity
    models.Team = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PlayerManager/api/team/",
        idAttribute: 'id',
        defaults: {
            name: ""
        },
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/PlayerManager/api/team/';
            }
            if (method == 'update') {
                options.url = 'http://localhost:8080/PlayerManager/api/team/';
            }
    ...
    models.TeamCollection = Backbone.Collection.extend({
        model: models.Team,
        url: "http://localhost:8080/PlayerManager/api/team/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
// Model for Player entity
    models.Player = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PlayerManager/api/player/",
        idAttribute: 'id',
        defaults: {
            name: ""
        },
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/PlayerManager/api/player/';
            }
            if (method == 'update') {
                options.url = 'http://localhost:8080/PlayerManager/api/player/';
            }
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        details: function(id) {
            if (this.collection) {
                ...
                var players = new models.PlayerCollection({id: id});
                players.url = "http://localhost:8080/PlayerManager/api/team/" + id + "/players";
                players.fetch({success: function(){
                    ...
                    $('#details').html(self.listView.render().el);
                }});
            } else {
                this.requestedId = id;
                this.list();
